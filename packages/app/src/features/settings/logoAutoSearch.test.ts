...
describe('autoSearchLogos', () => {
  it('gemmer det fundne som eget valg, husker det forgaeves, og lader eget valg vaere', async () => {
    ...
    expect(await handle.result).toEqual({ found: 1, tried: 2, skipped: 0, withBids: 1, problem: null });
  ...
  it('springer dem over der blev soegt for nylig', async () => {
    ...
    expect(await handle.result).toEqual({ found: 0, tried: 1, skipped: 1, withBids: 0, problem: null });
  });
