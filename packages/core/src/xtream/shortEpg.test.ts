...
describe('mapShortEpg', () => {
  it('afkoder titel og beskrivelse og omsaetter epoch-sekunder', () => {
    const [programme] = mapShortEpg('247634', { epg_listings: [listing()] });
  ...
  it('bruger kalderens stream_id, ikke svarets channel_id', () => {
    ...
    const [programme] = mapShortEpg('247634', {
  ...
  it('accepterer et bart array uden epg_listings-indpakning', () => {
    expect(mapShortEpg('1', [listing()])).toHaveLength(1);
  });

  it('accepterer tidsstempler som tal', () => {
    const [programme] = mapShortEpg('1', [
  ...
  it('springer poster over uden laesbar titel', () => {
    expect(mapShortEpg('1', [listing({ title: undefined })])).toEqual([]);
    expect(mapShortEpg('1', [listing({ title: '' })])).toEqual([]);
    // Ikke base64: bliver til en klump vi ikke vil vise som programtitel.
    expect(mapShortEpg('1', [listing({ title: '!!!' })])).toEqual([]);
  });

  it('beholder posten naar kun beskrivelsen er beskadiget', () => {
    const [programme] = mapShortEpg('1', [listing({ description: '!!!' })]);
  ...
  it('springer poster over uden brugbare tidsstempler', () => {
    expect(mapShortEpg('1', [listing({ start_timestamp: undefined })])).toEqual([]);
    expect(mapShortEpg('1', [listing({ stop_timestamp: 'i morgen' })])).toEqual([]);
    expect(mapShortEpg('1', [listing({ start_timestamp: '0' })])).toEqual([]);
  });
  ...
  it('springer poster over hvor sluttidspunktet ikke ligger efter starten', () => {
    ...
    expect(mapShortEpg('1', [listing({ stop_timestamp: '1757088000' })])).toEqual([]);
    expect(mapShortEpg('1', [listing({ stop_timestamp: '1757087000' })])).toEqual([]);
  });

  it('taaler svar der slet ikke ligner en programoversigt', () => {
    for (const raw of [null, undefined, 42, 'fejl', {}, { epg_listings: 'nej' }]) {
      expect(mapShortEpg('1', raw)).toEqual([]);
    }
    expect(mapShortEpg('1', [null, 7, ['nej'], listing()])).toHaveLength(1);
  });
});
